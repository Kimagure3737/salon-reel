...
2. **LP公開ブロッカー＝運営者情報の未記入**: src/lp/index.html 500〜502行目の事業者名・代表者・連絡先が空欄（`（記入してください）`）→実データ反映が必須。Instagramユーザー名は候補 `salon_reel` を全CTAリンクに反映済み（旧 `__YOUR_ACCOUNT__` は解消）だが、`@salon_reel` の実機取得確認は未了。反映後に実機最終確認（docs/LP_公開前チェックリスト.md）
